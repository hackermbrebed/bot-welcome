...
def create_inline_keyboard(config):
    ...
    dengan pola tata letak bergantian: 2 tombol, lalu 1 tombol, dst (2-1-2-1...).
    """
    keyboard_buttons = []
    i = 0
    
    # Konversi list [text, url] menjadi objek InlineKeyboardButton yang valid
    inline_btns = []
    ...
    while i < len(inline_btns):
        # Cek Pola: Pola bergantian dimulai 2-1-2-1...
        
        # --- Kondisi 1: Baris 2 Tombol ---
        # Jika ada minimal 2 tombol tersisa DAN giliran membuat baris 2 (indeks baris genap: 0, 2, 4...)
        if (len(inline_btns) - i >= 2) and (len(keyboard_buttons) % 2 == 0):
            keyboard_buttons.append(inline_btns[i:i+2])
            i += 2
        
        # --- Kondisi 2: Baris 1 Tombol ---
        # Dijalankan jika sisa tombol 1 (kasus terakhir), ATAU jika giliran membuat baris 1 (indeks baris ganjil: 1, 3, 5...)
...
async def done_set_button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not new_config:
        ...
        # Menggunakan fungsi create_inline_keyboard yang sudah dimodifikasi
